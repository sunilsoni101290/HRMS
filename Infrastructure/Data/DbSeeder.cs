...
namespace Infrastructure.Data
{
    public class DbSeeder
    {
        public static async Task SeedAsync(ApplicationDbContext context)
        {
            ...
            // Daily Work Entry module - Job Types/Work Activities/Idle-
            // Downtime Reasons/Document Statuses from the Excel reference.
            // Idempotent (upsert-by-Name/Code, same convention as the
            // LeaveTypes block below) - safe to run on every startup.
            await WorkTrackingMasterSeeder.SeedAsync(context, tenantId);
    ...
    public static class AppFeatureSeeder
    {
            ...
            string tenantId)
        {
            ...
            // eSSL SQL Integration - Settings/Test Connection/Sync Now tab +
            // Sync Logs tab + Unmapped Employees tab, all one screen/feature
            // (View to see it, Edit to change settings and trigger syncs).
            Def("eSSL SQL Integration", AppFeatureConstants.ESSL_INTEGRATION,
                AppFeatureConstants.ESSL_INTEGRATION_CONTROLLER, AppFeatureConstants.ESSL_INTEGRATION_ACTION,
                AppFeatureConstants.ATTENDANCE_MANAGEMENT, "bi bi-database-fill-gear", AppFeatureType.Master, 70,
                canEdit: true);
            ...
            // ---------------- EMPLOYEE WORK MANAGEMENT (Daily Work Entry module) ----------------
            // New top-level HRMS menu group, per explicit user confirmation
            // during this feature's design - Create/MyEntries/Details are
            // self-service; PendingApproval/Approve/Reject are reachable by
            // a Reporting Manager logged in under the plain self-service
            // role too (see EssRestrictionAttribute's remarks on
            // DailyWorkEntryController), so canApprove is granted here
            // rather than gating the whole feature to Admin/HR.
            ...
            // Reason/Document Status) - one tabbed HR/Admin-only screen,
    ...
    /// <summary>
    /// Daily Work Entry module master data - Job Types and their Work
    /// Activities, Idle/Downtime reasons, and Document Statuses, straight
    /// from the "Daily Work Entry.xlsx" reference (Job type and work done /
    /// Sheet2 / Reports-Only for assigned sheets). Idempotent: every insert
    /// is guarded by an existence check on Name (scoped by JobTypeId for
    /// activities), so this is safe to run on every startup and never
    /// duplicates a row that's already there - same convention as
    /// DbSeeder.SeedAsync's LeaveTypes block. Spec section 33.
    /// </summary>
    public static class WorkTrackingMasterSeeder
    {
        public static async Task SeedAsync(ApplicationDbContext context, string tenantId)
        {
            var jobTypes = await SeedJobTypesAsync(context, tenantId);
            await SeedWorkActivitiesAsync(context, tenantId, jobTypes);
            await SeedWorkEntryReasonsAsync(context, tenantId);
            await SeedDocumentStatusesAsync(context, tenantId);
        }

        private static async Task<Dictionary<string, JobType>> SeedJobTypesAsync(ApplicationDbContext context, string tenantId)
        {
            // (Name, DisplayOrder, HasDisciplines) - order matches the
            // Excel's "Job type and work done" sheet's Sr. No column 1-8,
            // plus "Others.." as a 9th, always-last catch-all.
            var defs = new (string Name, int Order, bool HasDisciplines)[]
            {
                ("Structural Steel", 1, false),
                ("Equipment", 2, false),
                ("Piping", 3, false),
                ("Skids Packages", 4, true),
                ("Project Management", 5, false),
                ("Estimation Work", 6, false),
                ("FEA", 7, false),
                ("PE Services", 8, false),
                ("Others", 9, false),
            };

            var existing = await context.JobTypes
                .Where(x => x.TenantId == tenantId)
                .ToListAsync();

            var byName = existing.ToDictionary(x => x.Name, x => x);

            foreach (var (name, order, hasDisciplines) in defs)
            {
                if (byName.ContainsKey(name)) continue;

                var entity = new JobType
                {
                    Id = IDManager.GetNewId(new JobType()),
                    TenantId = tenantId,
                    Name = name,
                    DisplayOrder = order,
                ...
                await context.JobTypes.AddAsync(entity);
                byName[name] = entity;
            }
        ...
        private static async Task SeedWorkActivitiesAsync(ApplicationDbContext context, string tenantId, Dictionary<string, JobType> jobTypes)
        {
            var existing = await context.WorkActivities
                .Where(x => x.TenantId == tenantId)
                .ToListAsync();

            // (JobTypeId, SkidsDiscipline, Name) -> already exists.
            bool Exists(string jobTypeId, SkidsDiscipline? discipline, string name) =>
                existing.Any(x => x.JobTypeId == jobTypeId && x.SkidsDiscipline == discipline && x.Name == name);

            var toAdd = new List<WorkActivity>();

            void Add(string jobTypeName, SkidsDiscipline? discipline, string name, WorkCategory category, int order,
                bool requiresReason = false, bool allowFreeTextOther = false)
            {
                if (!jobTypes.TryGetValue(jobTypeName, out var jobType)) return;
                if (Exists(jobType.Id, discipline, name)) return;

                toAdd.Add(new WorkActivity
                {
                    Id = IDManager.GetNewId(new WorkActivity()),
                    TenantId = tenantId,
                    JobTypeId = jobType.Id,
                    SkidsDiscipline = discipline,
                    Name = name,
                    WorkCategory = category,
                    DisplayOrder = order,
                    RequiresReason = requiresReason,
                    AllowFreeTextOther = allowFreeTextOther,
                    IsActive = true,
                    CreatedBy = "System",
                    CreatedOn = DateTime.UtcNow
                });
            ...
            // Shared "tail" of activities repeated identically across
            // Equipment/Piping/Skids-disciplines/FEA in the Excel
            // (Documentation..Others), so each Job Type's block below only
            // adds its own DISCIPLINE-SPECIFIC activities before calling
            // this helper for the common tail.
            void AddCommonTail(string jobTypeName, SkidsDiscipline? discipline, int startOrder, bool includeProjectManagement)
            {
                int o = startOrder;
                Add(jobTypeName, discipline, "Documentation", WorkCategory.Indirect, o++);
                Add(jobTypeName, discipline, "Meetings", WorkCategory.Indirect, o++);
                Add(jobTypeName, discipline, "Client - Coordination", WorkCategory.Indirect, o++);
                Add(jobTypeName, discipline, "Revision Works (Internal)", WorkCategory.Direct, o++);
                Add(jobTypeName, discipline, "Revision Works (Due to Client scope/IFC change)", WorkCategory.Direct, o++);
                Add(jobTypeName, discipline, "Downtime", WorkCategory.Downtime, o++, requiresReason: true);
                Add(jobTypeName, discipline, "On-Site", WorkCategory.Indirect, o++);
                if (includeProjectManagement)
                    Add(jobTypeName, discipline, "Project Management", WorkCategory.Indirect, o++);
                Add(jobTypeName, discipline, "Others", WorkCategory.Direct, o++, allowFreeTextOther: true);
            }

            // ---- Structural Steel ----
            {
                int o = 1;
                Add("Structural Steel", null, "Structure Design", WorkCategory.Direct, o++);
                Add("Structural Steel", null, "TEKLA Model Preparation", WorkCategory.Direct, o++);
                Add("Structural Steel", null, "TEKLA Erection Dwg Preparation", WorkCategory.Direct, o++);
                Add("Structural Steel", null, "TEKLA Fab/Part Dwg Preparation", WorkCategory.Direct, o++);
                Add("Structural Steel", null, "TEKLA Reports", WorkCategory.Direct, o++);
                Add("Structural Steel", null, "TEKLA-Dwg Checking", WorkCategory.Direct, o++);
                Add("Structural Steel", null, "Documentation", WorkCategory.Indirect, o++);
                Add("Structural Steel", null, "Meetings", WorkCategory.Indirect, o++);
                Add("Structural Steel", null, "Client - Coordination", WorkCategory.Indirect, o++);
                Add("Structural Steel", null, "Revision Works (Internal)", WorkCategory.Direct, o++);
                Add("Structural Steel", null, "Revision Works (Due to Client scope/IFC change)", WorkCategory.Direct, o++);
                Add("Structural Steel", null, "Downtime", WorkCategory.Downtime, o++, requiresReason: true);
                Add("Structural Steel", null, "On-Site", WorkCategory.Indirect, o++);
                Add("Structural Steel", null, "Others", WorkCategory.Direct, o++, allowFreeTextOther: true);
            }

            // ---- Equipment ----
            {
                int o = 1;
                Add("Equipment", null, "Design Calculation", WorkCategory.Direct, o++);
                Add("Equipment", null, "GA Drawings", WorkCategory.Direct, o++);
                Add("Equipment", null, "Part Fabrication Drawings", WorkCategory.Direct, o++);
                // "Materila Take-Off" is the Excel's own spelling
                // (typo for "Material Take-Off") - preserved verbatim as
                // the display name per spec section 7's "preserve the
                // Excel terminology" instruction, since silently
                // correcting it could make this master data disagree with
                // what users see in the source spreadsheet during rollout.
                Add("Equipment", null, "Material Take-Off", WorkCategory.Direct, o++);
                // TDC - organization-specific abbreviation, meaning not
                // established anywhere in this codebase (spec section 7 /
                // 26) - preserved as-is rather than guessed.
                Add("Equipment", null, "TDC", WorkCategory.Direct, o++);
                AddCommonTail("Equipment", null, o, includeProjectManagement: true);
            }

            // ---- Piping ----
            {
                int o = 1;
                Add("Piping", null, "Stress Analysis", WorkCategory.Direct, o++);
                // Excel groups "Piping GA/Isometric/P&ID etc" as one row -
                // kept as a single activity (spec section 8: "support that
                // grouping while keeping the data model flexible") rather
                // than inventing 3 separate rows the Excel doesn't define.
                Add("Piping", null, "Piping GA/Isometric/P&ID", WorkCategory.Direct, o++);
                Add("Piping", null, "Material Take-Off", WorkCategory.Direct, o++);
                Add("Piping", null, "TDC", WorkCategory.Direct, o++);
                AddCommonTail("Piping", null, o, includeProjectManagement: true);
            }

            // ---- Skids Packages (discipline-scoped: Piping/Equipment/Structural reuse those lists; E&I is its own) ----
            {
                Add("Skids Packages", SkidsDiscipline.Piping, "Stress Analysis", WorkCategory.Direct, 1);
                Add("Skids Packages", SkidsDiscipline.Piping, "Piping GA/Isometric/P&ID", WorkCategory.Direct, 2);
                Add("Skids Packages", SkidsDiscipline.Piping, "Material Take-Off", WorkCategory.Direct, 3);
                Add("Skids Packages", SkidsDiscipline.Piping, "TDC", WorkCategory.Direct, 4);
                AddCommonTail("Skids Packages", SkidsDiscipline.Piping, 5, includeProjectManagement: true);

                Add("Skids Packages", SkidsDiscipline.Equipment, "Design Calculation", WorkCategory.Direct, 1);
                Add("Skids Packages", SkidsDiscipline.Equipment, "GA Drawings", WorkCategory.Direct, 2);
                Add("Skids Packages", SkidsDiscipline.Equipment, "Part Fabrication Drawings", WorkCategory.Direct, 3);
                Add("Skids Packages", SkidsDiscipline.Equipment, "Material Take-Off", WorkCategory.Direct, 4);
                Add("Skids Packages", SkidsDiscipline.Equipment, "TDC", WorkCategory.Direct, 5);
                AddCommonTail("Skids Packages", SkidsDiscipline.Equipment, 6, includeProjectManagement: true);

                Add("Skids Packages", SkidsDiscipline.Structural, "Structure Design", WorkCategory.Direct, 1);
                Add("Skids Packages", SkidsDiscipline.Structural, "TEKLA Model Preparation", WorkCategory.Direct, 2);
                Add("Skids Packages", SkidsDiscipline.Structural, "TEKLA Erection Dwg Preparation", WorkCategory.Direct, 3);
                Add("Skids Packages", SkidsDiscipline.Structural, "TEKLA Fab/Part Dwg Preparation", WorkCategory.Direct, 4);
                Add("Skids Packages", SkidsDiscipline.Structural, "TEKLA Reports", WorkCategory.Direct, 5);
                Add("Skids Packages", SkidsDiscipline.Structural, "TEKLA-Dwg Checking", WorkCategory.Direct, 6);
                AddCommonTail("Skids Packages", SkidsDiscipline.Structural, 7, includeProjectManagement: false);

                // E&I (Electrical & Instrumentation) - the Excel gives only
                // "E&I Design & Drawings" as its discipline-specific
                // activity before the same common tail.
                Add("Skids Packages", SkidsDiscipline.EAndI, "E&I Design & Drawings", WorkCategory.Direct, 1);
                AddCommonTail("Skids Packages", SkidsDiscipline.EAndI, 2, includeProjectManagement: true);
            }

            // ---- Project Management / Estimation Work / PE Services ----
            // The Excel gives these Job Types a single named activity each
            // (Sr. No 5/6/8's "Work Done Today" column), rather than the
            // full Direct/Indirect breakdown the other Job Types get -
            // preserved as-is rather than inventing additional activities
            // the source document doesn't define.
            Add("Project Management", null, "Project Management", WorkCategory.Indirect, 1);
            Add("Estimation Work", null, "Estimation Work", WorkCategory.Direct, 1);
            Add("PE Services", null, "Project Management", WorkCategory.Indirect, 1);
            Add("PE Services", null, "Documentation", WorkCategory.Indirect, 2);

            // ---- FEA ----
            {
                int o = 1;
                Add("FEA", null, "FEA - Modeling & Analysis", WorkCategory.Direct, o++);
                Add("FEA", null, "Report Preparation", WorkCategory.Direct, o++);
                AddCommonTail("FEA", null, o, includeProjectManagement: true);
            }

            if (toAdd.Count > 0)
            {
                await context.WorkActivities.AddRangeAsync(toAdd);
                await context.SaveChangesAsync();
            }
        }

        private static async Task SeedWorkEntryReasonsAsync(ApplicationDbContext context, string tenantId)
        {
            var existing = await context.WorkEntryReasons
                .Where(x => x.TenantId == tenantId)
                .Select(x => new { x.Category, x.Name })
                .ToListAsync();

            bool Exists(WorkEntryReasonCategory cat, string name) => existing.Any(x => x.Category == cat && x.Name == name);

            var toAdd = new List<WorkEntryReason>();

            void Add(WorkEntryReasonCategory category, string name, int order)
            {
                if (Exists(category, name)) return;

                toAdd.Add(new WorkEntryReason
                {
                    Id = IDManager.GetNewId(new WorkEntryReason()),
                    TenantId = tenantId,
                    Category = category,
                    Name = name,
                    DisplayOrder = order,
            ...
            // Excel's Sheet2 lists Machine breakdown/Power failure/System-
            // ERP downtime/No work assigned/Training Hours/Administrative
            // Hours all under one "Idle Hours" header - split here per the
            // written spec's explicit Idle-vs-Downtime distinction (spec
            // sections 11/12) rather than the Excel's own grouping:
            // equipment/system failures are Downtime, everything else
            // (no work assigned, waiting, training, admin) is Idle.
            Add(WorkEntryReasonCategory.Idle, "No Work Assigned", 1);
            Add(WorkEntryReasonCategory.Idle, "Waiting for Input", 2);
            Add(WorkEntryReasonCategory.Idle, "Waiting for Approval", 3);
            Add(WorkEntryReasonCategory.Idle, "Training Hours", 4);
            Add(WorkEntryReasonCategory.Idle, "Administrative Hours", 5);
            Add(WorkEntryReasonCategory.Idle, "Other", 6);

            Add(WorkEntryReasonCategory.Downtime, "Machine Breakdown", 1);
            Add(WorkEntryReasonCategory.Downtime, "Power Failure", 2);
            Add(WorkEntryReasonCategory.Downtime, "System/ERP Downtime", 3);
            Add(WorkEntryReasonCategory.Downtime, "Other", 4);

            if (toAdd.Count > 0)
            {
                await context.WorkEntryReasons.AddRangeAsync(toAdd);
                await context.SaveChangesAsync();
            }
        }

        private static async Task SeedDocumentStatusesAsync(ApplicationDbContext context, string tenantId)
        {
            var existing = await context.DocumentStatuses
                .Where(x => x.TenantId == tenantId)
                .Select(x => x.Code)
                .ToListAsync();

            var toAdd = new List<DocumentStatus>();

            // Codes preserved exactly as they appear in the Excel's
            // "Reports-Only for assigned" sheet. WNS's business meaning is
            // not established anywhere in this codebase (spec section 26) -
            // its DisplayName is deliberately left equal to its Code rather
            // than inventing a definition; update it once the real meaning
            // is confirmed.
            var defs = new (string Code, string DisplayName, int Order)[]
            {
                ("ED-IFA", "ED-IFA (Engineering Drawing - Issued for Approval)", 1),
                ("FD-IFA", "FD-IFA (Fabrication Drawing - Issued for Approval)", 2),
                ("IFC", "IFC (Issued for Construction)", 3),
                ("ED-Working", "ED-Working (Engineering Drawing - Working)", 4),
                ("FD-Working", "FD-Working (Fabrication Drawing - Working)", 5),
                ("As-Built", "As-Built", 6),
                ("CLOSED", "Closed", 7),
                ("WNS", "WNS", 8),
            };

            foreach (var (code, displayName, order) in defs)
            {
                if (existing.Contains(code)) continue;

                toAdd.Add(new DocumentStatus
                {
                    Id = IDManager.GetNewId(new DocumentStatus()),
                    TenantId = tenantId,
                    Code = code,
            ...
            if (toAdd.Count > 0)
            {
                await context.DocumentStatuses.AddRangeAsync(toAdd);
                await context.SaveChangesAsync();
            }
        }
    }
