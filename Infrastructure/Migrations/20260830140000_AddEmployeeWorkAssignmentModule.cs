...
namespace Infrastructure.Migrations
{
    /// <inheritdoc />
    /// <remarks>
    /// Idempotent (IF NOT EXISTS-guarded raw SQL), same convention as every
    /// other migration in this module - adds the two new
    /// EmployeeWorkAssignment(History) tables plus the new
    /// AssignmentId/WorkDoneToday/AdhocReason columns on DailyWorkEntries.
    /// No existing table/column is altered or removed.
    /// </remarks>
    public partial class AddEmployeeWorkAssignmentModule : Migration
    {
...
");
        ...
        {
            migrationBuilder.Sql("IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.DailyWorkEntries') AND name = 'AdhocReason') ALTER TABLE [dbo].[DailyWorkEntries] DROP COLUMN [AdhocReason];");
            migrationBuilder.Sql("IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.DailyWorkEntries') AND name = 'WorkDoneToday') ALTER TABLE [dbo].[DailyWorkEntries] DROP COLUMN [WorkDoneToday];");
            migrationBuilder.Sql("IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_DailyWorkEntries_AssignmentId' AND object_id = OBJECT_ID('dbo.DailyWorkEntries')) DROP INDEX [IX_DailyWorkEntries_AssignmentId] ON [dbo].[DailyWorkEntries];");
            migrationBuilder.Sql("IF EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_DailyWorkEntries_EmployeeWorkAssignments_AssignmentId') ALTER TABLE [dbo].[DailyWorkEntries] DROP CONSTRAINT [FK_DailyWorkEntries_EmployeeWorkAssignments_AssignmentId];");
            migrationBuilder.Sql("IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.DailyWorkEntries') AND name = 'AssignmentId') ALTER TABLE [dbo].[DailyWorkEntries] DROP COLUMN [AssignmentId];");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[EmployeeWorkAssignmentHistories];");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[EmployeeWorkAssignments];");
        }
    }
}
